...
const ExportPanel = ({
  ...
  const [loadingLabel, setLoadingLabel] = useState('Criando vídeo');
  ...
  useEffect(() => () => {
    loadingTimelineRef.current.forEach((timerId) => window.clearTimeout(timerId));
  ...
  const startCreatingVideoTimeline = () => {
    clearLoadingTimeline();
    setLoadingLabel('Criando vídeo');

    const almostThereTimer = window.setTimeout(() => {
      setLoadingLabel((currentLabel) => (currentLabel === 'Baixando vídeo...' ? currentLabel : 'Quase lá'));
    }, 30000);

    const backToCreatingTimer = window.setTimeout(() => {
      setLoadingLabel((currentLabel) => (currentLabel === 'Baixando vídeo...' ? currentLabel : 'Criando vídeo'));
    }, 50000);

    loadingTimelineRef.current = [almostThereTimer, backToCreatingTimer];
  };
