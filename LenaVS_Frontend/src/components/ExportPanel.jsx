...
const resolveMediaReferenceForExport = (mediaFiles = {}, mediaMetadata = {}, fieldName) => {
  const metadataEntry = mediaMetadata?.[fieldName] || {};

  const value = metadataEntry.storagePath
  ...
  return value
    ? {
      value,
      bucket: metadataEntry.bucket || null,
    }
    : null;
};
...
const ExportPanel = ({
  ...
  const [videoFormat, setVideoFormat] = useState(initialVideoFormat);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const backgroundVideoSource = resolveMediaReferenceForExport(mediaFiles, mediaMetadata, 'video');
      const backgroundImageSource = resolveMediaReferenceForExport(mediaFiles, mediaMetadata, 'imagem');
      const audioSource = effectiveAudioType === 'original'
        ? resolveMediaReferenceForExport(mediaFiles, mediaMetadata, 'musicaOriginal')
        : resolveMediaReferenceForExport(mediaFiles, mediaMetadata, 'musicaInstrumental');

      const response = await api.post('/video/generate', {
        projectName,
        audioType: effectiveAudioType,
        audioPath: audioSource?.value,
        audioBucket: audioSource?.bucket || undefined,
        backgroundType: backgroundVideoSource
          ? 'video'
          : backgroundImageSource
          ? 'image'
          : 'color',
        backgroundPath: backgroundVideoSource?.value || backgroundImageSource?.value,
        backgroundBucket: backgroundVideoSource?.bucket || backgroundImageSource?.bucket || undefined,
        backgroundColor,
        mediaAnimation,
        stanzas,
        videoFormat,
