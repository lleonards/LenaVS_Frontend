...
import { DEFAULT_TIMECODE } from '../utils/timecode';
  ...
  const handleLyricsUpload = async (e) => {
    ...
    try {
      ...
      const stanzasData = response.data.stanzas.map((text, idx) => ({
        id: idx, text,
        startTime: DEFAULT_TIMECODE, endTime: DEFAULT_TIMECODE,
      ...
      <div className="auto-sync-section">
        ...
        <div className="auto-sync-hint">
          {isSyncing
            ...
            : canSync
            ? uploaded.musicaInstrumental
              ? 'Sincroniza usando a música original, cria uma guia vocal interna e respeita as estrofes já separadas no editor'
              : 'Sincroniza os tempos usando a música original + análise vocal'
            : lockMessage}
        </div>
      </div>
