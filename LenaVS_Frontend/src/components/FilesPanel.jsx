...
import { Upload, Type, X, Wand2, Lock } from 'lucide-react';
...
const FilesPanel = ({
  ...
  onFilesUploaded,
  onAutoSync,
  onAudioDurationRead   // ← novo: informa duração local ao pai
}) => {
  ...
  const lockMessage = useMemo(() => {
    const hasAudio = uploaded.musicaOriginal;
    const hasLyrics = uploaded.letraArquivo || uploaded.letraManual;
    if (!hasAudio && !hasLyrics) return 'Faça upload da Música Original e da Letra';
    if (!hasAudio) return 'Faça upload da Música Original';
    if (!hasLyrics) return 'Faça upload ou cole a Letra';
    return '';
  }, [uploaded]);
      ...
      <div className="auto-sync-section">
        ...
        >
          <button
            ...
            onClick={() => canSync && onAutoSync && onAutoSync()}
        ...
        <div className="auto-sync-hint">
          {canSync
            ? 'Distribui os tempos das estrofes com base na música original'
            : lockMessage
          }
        </div>
      </div>
