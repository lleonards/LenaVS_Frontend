...
import { Upload, Type, X, Loader2, Sparkles, Zap } from 'lucide-react';
...
const FilesPanel = ({
  ...
  const [lastSyncMethod, setLastSyncMethod] = useState(null);
  ...
  // URL usada APENAS para sincronização — sempre prefere a música original (com voz)
  // O Whisper precisa da voz para encontrar os timecodes das estrofes
  const syncAudioUrl = useMemo(
    () => mediaFiles?.musicaOriginal || mediaFiles?.musicaInstrumental || null,
    [mediaFiles]
  );

  const effectiveStanzaCount = Array.isArray(stanzas) ? stanzas.length : stanzaCount;
  const canSync = Boolean(syncAudioUrl) && effectiveStanzaCount > 0;
  ...
  const handleAutoSync = async () => {
    if (!canSync || anyBusy || !isAuthenticated) return;

    const currentStanzas = Array.isArray(stanzas) ? stanzas : [];
    // Duração da música original — usada no fallback proporcional caso Whisper falhe
    const audioDuration = normalizeDurationValue(
      mediaMetadata?.musicaOriginal?.duration ?? mediaMetadata?.[effectiveAudioField]?.duration
    ) || null;

    try {
      setIsSyncing(true);

      const response = await api.post('/lyrics/sync', {
        audioUrl: syncAudioUrl,
        stanzas: currentStanzas.map((s) => ({ id: s.id, text: s.text })),
        audioDuration,
      });

      const synced = response.data?.stanzas;
      const method = response.data?.method || 'unknown';

      if (!Array.isArray(synced) || synced.length === 0) {
        throw new Error('O servidor não retornou estrofes sincronizadas.');
      }

      // Converte segundos (float) → MM:SS e mescla de volta nas estrofes originais
      const syncedMap = new Map(synced.map((s) => [s.id, s]));

      const updatedStanzas = currentStanzas.map((stanza) => {
        const syncedStanza = syncedMap.get(stanza.id);
        if (!syncedStanza) return stanza;

        const startSec = syncedStanza.startTime;
        const endSec = syncedStanza.endTime;
        const hasStart = typeof startSec === 'number' && Number.isFinite(startSec);
        const hasEnd = typeof endSec === 'number' && Number.isFinite(endSec);

        return {
          ...stanza,
          startTime: hasStart ? formatFixedTimecode(Math.round(startSec)) : stanza.startTime,
          endTime: hasEnd ? formatFixedTimecode(Math.round(endSec)) : stanza.endTime,
          hasManualStart: hasStart,
          hasManualEnd: hasEnd,
        };
      });

      setLastSyncMethod(method);
      onAutoSync?.(updatedStanzas);

      const methodLabel = method === 'whisper' ? 'Whisper AI' : 'distribuição proporcional';
      onNotify?.({
        type: 'success',
        title: 'Sincronização concluída',
        message: `Estrofes sincronizadas via ${methodLabel}. Ajuste manualmente se necessário.`,
      });
    } catch (error) {
      console.error('[FilesPanel] Erro na sincronização automática:', error);
      onNotify?.({
        type: 'error',
        title: 'Falha na sincronização',
        message: error.response?.data?.error || error.message || 'Erro ao sincronizar estrofes.',
      });
    } finally {
      setIsSyncing(false);
    }
    ...
    <div className="files-panel" ref={panelRef}>
      ...
      <div className="auto-sync-section">
        <div className="auto-sync-wrapper">
          <button
            type="button"
            className={`auto-sync-btn ${canSync ? 'unlocked' : 'locked'}`}
            onClick={handleAutoSync}
            disabled={!canSync || anyBusy}
            title={
              !syncAudioUrl
                ? 'Faça o upload da música original primeiro'
                : effectiveStanzaCount === 0
                  ? 'Adicione a letra primeiro'
                  : 'Sincronizar estrofes com a música original (com voz) via Whisper'
            }
          >
            {isSyncing ? (
              <>
        ...
        <p className="auto-sync-hint">
          {isSyncing
            ? 'Analisando a música… isso pode levar alguns segundos.'
            : lastSyncMethod === 'whisper'
              ? '✓ Sincronizado via Whisper AI'
              : lastSyncMethod === 'proportional'
                ? '✓ Sincronizado (ajuste fino manual recomendado)'
                : !syncAudioUrl
                  ? 'Upload a música para habilitar a sincronização.'
                  : effectiveStanzaCount === 0
                    ? 'Adicione a letra para habilitar a sincronização.'
                    : `${effectiveStanzaCount} estrofe${effectiveStanzaCount !== 1 ? 's' : ''} pronta${effectiveStanzaCount !== 1 ? 's' : ''} para sincronizar.`}
        </p>
      </div>
