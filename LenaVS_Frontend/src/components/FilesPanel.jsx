  ...
  useEffect(() => {
    ...
    Promise.allSettled(
      ...
      results.forEach((result) => {
        ...
        metadataPatch[field] = normalizeUploadedMediaMetadataEntry({ kind, duration }, kind);
      });
    ...
    ? createPortal(
      ...
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Painel de animações de mídia"
          onMouseDown={(event) => event.stopPropagation()}
        >
          <div className="files-animation-popover__header">
            <div>
              <span className="files-animation-popover__eyebrow">Painel compacto</span>
              <strong>Animações de mídia</strong>
              <p>
                Ajuste entrada, saída e loop sem corte visual. O painel agora abre fixo na viewport para não sumir ao rolar a tela.
              </p>
            </div>
          ...
          <div className={`files-animation-popover__body${shouldShowLoopTransition ? ' files-animation-popover__body--three' : ''}`}>
            ...
            {shouldShowLoopTransition
              ? renderAnimationControl('Loop do vídeo', 'loopTransition', 'loopTransitionDuration', {
                helper: 'Use quando o vídeo for menor que o áudio para suavizar a repetição.',
              })
              : null}
          </div>
        </div>
      ...
      <div className="upload-section">
        <label className={getUploadButtonClassName('musicaOriginal')} title={getUploadBlockedTitle('musicaOriginal')}>
          ...
          <div className="upload-btn-copy">
            ...
            <small>MP3, WAV, OGG, M4A, AAC, FLAC, WMA, OPUS e WEBA • até 15 min</small>
          </div>
        ...
        <label className={getUploadButtonClassName('video')} title={getUploadBlockedTitle('video')}>
          ...
          <div className="upload-btn-copy">
            ...
            <small>Vídeo: MP4, MOV, AVI, MKV, WEBM, M4V, MPEG, MPG, 3GP • Imagem: JPG, PNG, GIF, BMP, WEBP</small>
          </div>
          {renderBadge('video')}
          <input
            type="file"
