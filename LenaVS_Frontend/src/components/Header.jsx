...
const Header = ({ onOpenProjects = () => {} }) => {
  ...
  // Rotação de mensagens no topo: 60s mensagem principal, 30s mensagem secundária
  useEffect(() => {
    const DURATIONS = [60000, 30000]; // ms para cada mensagem
    let timeoutId;
    const schedule = (index) => {
      timeoutId = window.setTimeout(() => {
        const next = (index + 1) % 2;
        setRotatingMessageIndex(next);
        schedule(next);
      }, DURATIONS[index]);
    };
    schedule(0);
          ...
          {isCancelScheduled ? (
            ...
            <div className="header-inline-help-notice">
              <span className="header-inline-help-text">
                Ajude outros usuários publicando seus projetos na Biblioteca LenaVS.
              </span>
            </div>
          )}
        </div>
