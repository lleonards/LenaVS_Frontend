...
import { parseTimecode, formatTimecode, DEFAULT_TIMECODE } from '../utils/timecode';
import { createStanzaFromText, duplicateStanza, hasDefinedStartTime, isStanzaActiveAtTime } from '../utils/stanza';
...
const clampFontSize = (value) => {
  ...
  return Math.min(160, Math.max(12, Math.round(parsed)));
};
...
const LyricsEditorPanel = ({
  ...
  const updateTimeField = (id, field, rawValue, markConfigured = false) => {
    const parsed = parseTimecode(rawValue);
    const normalized = parsed === null ? DEFAULT_TIMECODE : formatTimecode(parsed);
  ...
  const formatCurrentTime = (seconds) => formatTimecode(seconds);
              ...
              <div className="lep-time-row">
                ...
                <div className="lep-field lep-field-sm">
                  ...
                  <input
                    ...
                    onChange={(e) => updateStanza(stanza.id, 'fontSize', clampFontSize(e.target.value))}
                    min="12"
                    max="160"
                  />
                </div>
