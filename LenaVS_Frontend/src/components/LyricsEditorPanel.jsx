...
import {
  ...
  STANZA_WIDTH_SCALE_MAX,
  STANZA_WIDTH_SCALE_MIN,
} from '../utils/stanza';
...
const LyricsEditorPanel = ({
  ...
  const handleWidthScaleChange = (stanzaId, rawValue) => {
    updateStanza(stanzaId, 'scaleX', normalizeStanzaWidthScale(rawValue));
  };
    ...
    <div className="lyrics-editor-panel">
      ...
      <div className="lep-list">
        {stanzas.map((stanza, idx) => {
          const active = isStanzaActiveAtTime(stanza, currentTime);
          const selected = stanza.id === selectedStanzaId;
          const needsStartTime = !hasDefinedStartTime(stanza);
          ...
          const safeWidthScale = normalizeStanzaWidthScale(stanza.scaleX);
          ...
          return (
            ...
            >
              ...
              <div className="lep-adjustments-grid">
                <div className="lep-adjust-card">
                  <div className="lep-adjust-card__head">
                    <span className="lep-adjust-card__title">Contorno</span>
                    <span className="lep-adjust-card__value">{safeOutlineWidth}</span>
                  </div>

                  <div className="lep-adjust-card__controls">
                    <input
                      className="lep-input lep-input-num lep-adjust-input"
                      type="number"
                      ...
                      onBlur={(e) => handleOutlineWidthChange(stanza.id, e.target.value)}
                ...
                <div className="lep-adjust-card">
                  <div className="lep-adjust-card__head">
                    <span className="lep-adjust-card__title">Largura</span>
                    <span className="lep-adjust-card__value">{safeWidthScale.toFixed(2)}x</span>
                  </div>

                  <div className="lep-width-scale-wrap">
                    <span className="lep-scale-hint">Mais magra</span>
                    <input
                      className="lep-slider"
                      type="range"
                      min={STANZA_WIDTH_SCALE_MIN}
                      max={STANZA_WIDTH_SCALE_MAX}
                      step="0.01"
                      value={safeWidthScale}
                      onChange={(e) => handleWidthScaleChange(stanza.id, e.target.value)}
                      title={`Largura da estrofe entre ${STANZA_WIDTH_SCALE_MIN}x e ${STANZA_WIDTH_SCALE_MAX}x`}
                    />
                    <span className="lep-scale-hint">Mais gorda</span>
                  </div>
                </div>
              </div>
