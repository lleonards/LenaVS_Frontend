...
const PreviewPanel = ({
  ...
  const { currentStanza, currentLineIndex } = useMemo(() => {
    ...
    // Encontra a estrofe cujo intervalo cobre o tempo atual
    ...
    // Determina a linha ativa dentro da estrofe (se tiver line-level timing)
    let lineIdx = -1;
    if (active.lines && active.lines.length > 0) {
      // Encontra a linha cujo intervalo cobre o tempo atual
      ...
      // Se nenhuma linha tem timing exato, calcula por posição relativa na estrofe
      if (lineIdx === -1) {
  ...
  const renderLyricsKaraoke = () => {
    ...
    // Modo karaokê: linha atual destacada, restantes em opacidade reduzida
    return (
      <div
        className={`lyrics-display lyrics-karaoke ${transitionClass}`}
        style={{ textAlign: currentStanza.alignment || 'center' }}
      >
        {lines.map((line, i) => {
          ...
          return (
            <div
              ...
              style={{
                ...
                textShadow:     isActive
                  ? `2px 2px 6px ${currentStanza.outlineColor || '#000'}, 0 0 20px ${currentStanza.outlineColor || '#000'}`
                  : `1px 1px 3px ${currentStanza.outlineColor || '#000'}`,
              }}
            >
              {line}
            </div>
  ...
  const getTextStyle = (stanza) => ({
    ...
    textShadow:     `2px 2px 4px ${stanza.outlineColor || '#000'}`,
