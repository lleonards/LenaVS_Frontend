...
const PreviewPanel = ({
  ...
  const currentStanza = useMemo(() => {
    ...
    const activeStanzas = stanzas.filter((stanza) => isStanzaActiveAtTime(stanza, currentTime));
    return activeStanzas.length ? activeStanzas[activeStanzas.length - 1] : null;
  }, [stanzas, currentTime]);
