...
const PreviewPanel = ({
  ...
  const currentTimelineIndex = useMemo(() => {
    if (!currentTimelineEntry) return -1;

    return resolvedTimeline.findIndex((entry) => entry?.stanza?.id === currentTimelineEntry.stanza?.id);
  }, [currentTimelineEntry, resolvedTimeline]);

  const nextVisibleTimelineEntry = useMemo(() => {
    if (currentTimelineIndex < 0) return null;

    for (let index = currentTimelineIndex + 1; index < resolvedTimeline.length; index += 1) {
    ...
    return null;
  }, [currentTimelineIndex, resolvedTimeline]);

  const shouldUseCurrentExitTransition = useMemo(() => {
    if (!currentTimelineEntry?.isVisible) {
      return false;
    }

    if (!nextVisibleTimelineEntry) {
      return true;
    }

    return nextVisibleTimelineEntry.displayStart > currentTimelineEntry.displayEnd + TIMELINE_EPSILON;
  }, [currentTimelineEntry, nextVisibleTimelineEntry]);
  ...
  const getTextStyle = (stanza) => {
    ...
    const isExitWindow = Boolean(
      isCurrentStanza
      && shouldUseCurrentExitTransition
    ...
    <div className="preview-panel">
      ...
      <div className="preview-controls">
        ...
        <button
          ...
          onClick={() => onAudioTypeChange(effectivePreviewAudioType === 'original' ? 'instrumental' : 'original')}
          disabled={Boolean(lockedAudioType) || (effectivePreviewAudioType === 'original' ? !mediaFiles?.musicaInstrumental : !mediaFiles?.musicaOriginal)}
          title={lockedAudioType ? 'Este projeto da biblioteca mantém apenas o áudio usado no projeto original.' : undefined}
        >
          <Music size={18} />
