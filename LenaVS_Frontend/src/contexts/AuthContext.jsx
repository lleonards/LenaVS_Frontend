// =============================================================================
// Importante: a causa raiz frequente do "não consigo logar nem cadastrar" é
// Supabase restrito por cota (HTTP 402 com "exceed_storage_size_quota" ou
// "restricted due to"). As rotas /auth/check-email e /auth/ensure-account
// propagam esse erro com payload { code: 'STORAGE_QUOTA_EXCEEDED' } ou
// mensagem similar para que o front exiba mensagem clara em vez do genérico.
// =============================================================================
...
const AUTH_ERROR_RULES = [
  {
    test: (msg, code) => /exceed_storage_size_quota|exceed storage size|storage quota/i.test(msg)
      || /exceed_storage_size_quota/i.test(String(code || ''))
      || /superase_limite_armazenamento|limite de armazenamento/i.test(msg),
    message:
      'O serviço do Supabase do LenaVS está temporariamente restrito por cota de armazenamento excedida. '
      + 'Libere espaço no painel do Supabase (ou remova o spend cap) e tente novamente em alguns minutos.',
  },
  {
    test: (msg) => /restricted due to|service is restricted/i.test(msg),
    message:
      'O serviço do Supabase do LenaVS está temporariamente restrito. '
      + 'Verifique a fatura do projeto no Supabase, remova o spend cap ou faça upgrade do plano.',
  },
  {
    test: (msg) => /billing|spend cap|plan upgrade|excedeu/i.test(msg),
    message:
      'O projeto Supabase atingiu o limite do plano gratuito. '
      + 'Remova o spend cap no painel do Supabase ou faça upgrade do plano.',
  },
];
...
const toFriendlyAuthError = (error, operation = 'login') => {
  ...
  for (const rule of AUTH_ERROR_RULES) {
    if (rule.test(message, code)) return new Error(rule.message);
  }
