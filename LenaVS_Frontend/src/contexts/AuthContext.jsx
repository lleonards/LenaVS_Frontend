...
export const AuthProvider = ({ children }) => {
  ...
  const signIn = async (
    ...
    // Login direto no Supabase (API v2: signInWithPassword).
    // O endpoint /auth/login do backend é mantido como fallback,
    // porque ele também registra o acesso no servidor.
    ...
    try {
      ...
      if (!directError && directData?.session) {
        returnedSession = directData.session;
      } else if (directError) {
        throw directError;
      }
    } catch (directError) {
      // Fallback: tenta pelo backend (mesma validação, sem alterar dados).
      const { data } = await api.post('/auth/login', {
        email: normalizedEmail,
        password,
      });
      returnedSession = data?.session || null;
    }
