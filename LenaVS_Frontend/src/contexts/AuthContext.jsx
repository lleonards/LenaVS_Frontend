...
export const AuthProvider = ({ children }) => {
  ...
  const deleteAccount = async ({ reason, feedback = '' } = {}) => {
    const payload = {
      reason: String(reason || '').trim(),
      feedback: String(feedback || '').trim(),
    };

    const { data } = await api.post('/user/delete-account', payload);

    try {
      await supabase.auth.signOut();
    } catch (error) {
      console.warn('Falha ao encerrar sessão após excluir conta:', error?.message || error);
    }
