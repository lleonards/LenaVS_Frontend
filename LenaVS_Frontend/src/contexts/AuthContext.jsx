...
const deriveProfileFromSessionUser = (sessionUser) => {
  ...
  return { displayName, avatarUrl, email: sessionUser?.email || null };
};
...
export const AuthProvider = ({ children }) => {
  ...
  const [cancellationSurveyPending, setCancellationSurveyPending] = useState(false);
  ...
  const resetLocalUserState = () => {
    ...
    setAvatarUrl(null);
    setUserEmail(null);
    setCancellationSurveyPending(false);
  };
  ...
  const applyUserSnapshot = (data, fallbackUser = null) => {
    ...
    setCancellationSurveyPending(data?.cancellation_survey_pending ?? false);
    return data ?? null;
  };
  ...
  const fetchUserData = async (userId, fallbackUser = session?.user ?? null) => {
    try {
      ...
      const { data, error } = await withTimeout(
        supabase
          .from('users')
          .select('credits, plan, subscription_status, unlimited_access_until, cancellation_survey_pending')
          .eq('id', userId)
          .maybeSingle(),
        AUTH_BOOT_TIMEOUT_MS,
  ...
  const submitCancellationFeedback = async ({ reason, feedback = '', dismissed = false } = {}) => {
    const payload = { dismissed };
    if (!dismissed) {
      payload.reason = String(reason || '').trim();
      payload.feedback = String(feedback || '').trim();
    }
    await api.post('/user/cancellation-feedback', payload);
    setCancellationSurveyPending(false);
  };
    ...
    const initializeAuth = async () => {
      try {
        if (!hasSupabaseConfig) {
          if (isMounted) { setSession(null); resetLocalUserState(); }
          return;
        }
        ...
        if (error) console.error('Erro getSession:', error.message);
  ...
  const signUp = async (email, password, name, countryCode = 'BR', acceptedLegal = false) => {
    ...
    const countryLabelMap = {
      BR: 'Brasil', US: 'Estados Unidos', CA: 'Canadá', AU: 'Austrália',
      NZ: 'Nova Zelândia', SG: 'Singapura', HK: 'Hong Kong', OTHER: 'Outros',
    };
  ...
  const hasUnlimitedAccess = useMemo(() => {
    ...
    if (untilDate) return untilDate.getTime() > Date.now();
