// O frontend mostra somente mensagens seguras e úteis ao usuário.
// Detalhes técnicos do Supabase ficam somente nos logs do servidor.
...
const toFriendlyAuthError = (error, operation = 'login') => {
  ...
  return new Error('Erro no sistema. Tente novamente mais tarde.');
};
...
export const AuthProvider = ({ children }) => {
  ...
  const signUp = async (email, password, name, countryCode = 'BR', acceptedLegal = false) => {
    ...
    try {
      ...
      // A confirmação deve ser obrigatória. Se a configuração do Supabase
      // devolver uma sessão, encerre-a para nunca liberar o editor sem
      ...
      return {
        ...
        message: 'Cadastro realizado. Enviamos um link de confirmação para seu e-mail. Confirme a conta antes de fazer login.',
      };
    } catch (supabaseError) {
      ...
      console.error('Erro no cadastro pelo Supabase:', supabaseError);
  ...
  const signIn = async (email, password) => {
    ...
    try {
      ...
      if (errorMessage.includes('email not confirmed') || errorCode.includes('email_not_confirmed')) {
        throw new Error('Confirme seu e-mail antes de entrar. Verifique também a pasta de spam.');
      } else {
