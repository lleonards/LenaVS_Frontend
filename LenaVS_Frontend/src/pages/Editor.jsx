  ...
  const requestPreviewSeek = useCallback((stanzaId, options = {}) => {
    ...
    if (!hasDefinedStartTime(stanza)) {
      if (notifyIfMissingStart) {
        setSyncStatus({
          type: 'warning',
          message: 'O preview só volta automaticamente quando o bloco tiver início e fim preenchidos.',
        });
      }
      return false;
    }
    ...
    if (start === null || end === null || end <= start) {
      if (notifyIfMissingStart) {
        setSyncStatus({
          type: 'warning',
          message: 'Preencha início e fim válidos antes de pedir o retorno automático do preview.',
        });
      }
      return false;
    }
