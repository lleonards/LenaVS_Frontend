...
const Editor = () => {
  ...
  // Rastreia se o projeto atual veio da biblioteca sem edições de conteúdo.
  // Quando verdadeiro, a exportação NÃO publica o projeto na biblioteca.
  // Torna-se false assim que o usuário editar qualquer conteúdo real (letras,
  // fundo, mídia, animação) — apenas alterar o nome não conta como edição.
  const [isLibraryForkNoEdits, setIsLibraryForkNoEdits] = useState(false);
  ...
  const handleLyricsProcessed = useCallback((processedStanzas) => {
    ...
    setSyncStatus(null);
    // O usuário fez upload de nova letra: isso é uma edição de conteúdo real
    setIsLibraryForkNoEdits(false);
  }, [previewMetrics]);
  ...
  const handleTapSync = useCallback((stanzaId, field) => {
    ...
    // Edição de timing é uma edição de conteúdo real
    setIsLibraryForkNoEdits(false);
  }, []);
  ...
  const openBlankProject = useCallback(() => {
    ...
    // Projeto novo: sem fork de biblioteca
    setIsLibraryForkNoEdits(false);
  }, []);
  ...
  const handleOpenProject = useCallback(async (project) => {
    await withProjectLoading(project.id, 'open', `Abrindo o projeto "${project.name}"...`, async () => {
      ...
      // Abrir projeto do histórico: não é fork de biblioteca
      setIsLibraryForkNoEdits(false);
    });
  ...
  const handleForkProject = useCallback(async (project) => {
    try {
      await withProjectLoading(project.id, 'fork', `Carregando a sua cópia de "${project.name}"...`, async () => {
        ...
        // Projeto veio da biblioteca sem nenhuma edição de conteúdo ainda.
        // Se o usuário exportar sem editar nada (apenas alterar o nome),
        // o projeto NÃO será publicado na biblioteca automaticamente.
        setIsLibraryForkNoEdits(true);
      });
    } catch (error) {
      ...
      >
        ...
        <div className="export-wrapper" ref={exportWrapperRef}>
          <ExportPanel
            ...
            onNotify={pushNotice}
            isLibraryForkNoEdits={isLibraryForkNoEdits}
          />
        </div>
      </div>
      ...
      {showPublishModal && publishModalProject ? (
        <div className="publish-modal-overlay" onClick={() => { if (!publishingProject) { setShowPublishModal(false); } }}>
          <div className="publish-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="publish-modal__desc">Escolha um nome público para o seu projeto. Ele aparecerá na Biblioteca para outros usuários.</p>
            <div className="publish-modal__field">
              ...
              <input
                ...
                placeholder="Ex: Culto Jovem — Tema Azul 2026"
                maxLength={60}
            ...
            <p className="publish-modal__hint">O nome interno "{publishModalProject.name}" permanece visível apenas para você.</p>
