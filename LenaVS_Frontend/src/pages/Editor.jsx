...
const Editor = () => {
  ...
  const [isPreviewVisibleOnMobile, setIsPreviewVisibleOnMobile] = useState(true);
  ...
  }, [currentProjectId, withProjectLoading]);
  ...
  }, [focusSelectedStanzaBlock, handleTapSync, navigateBetweenStanzas, selectedStanzaId]);
  ...
  useEffect(() => {
    if (layoutMode !== 'mobile' || !showLyricsEditor) {
      setIsPreviewVisibleOnMobile(true);
    ...
    if (!previewElement || !editorElement || typeof IntersectionObserver === 'undefined') {
      return undefined;
    }

    const thresholds = [0, 0.1, 0.2, 0.35, 0.6, 1];
    const previewObserver = new IntersectionObserver(
      ([entry]) => {
        setIsPreviewVisibleOnMobile(Boolean(entry?.isIntersecting && entry.intersectionRatio >= 0.35));
      },
    ...
    const editorObserver = new IntersectionObserver(
      ([entry]) => {
        setIsLyricsEditorVisibleOnMobile(Boolean(entry?.isIntersecting && entry.intersectionRatio >= 0.2));
      },
      { threshold: thresholds }
    );
