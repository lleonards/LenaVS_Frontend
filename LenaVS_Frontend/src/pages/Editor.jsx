...
const Editor = () => {
  ...
  const handleAutoSync = async () => {
    ...
    try {
      ...
      let response;
      try {
        // Agora: backend também pode RE-SEPARAR as estrofes (resegmentação)
        // para que cada bloco do editor represente uma estrofe "ideal" pro áudio.
        const lyricsText = stanzas.map(s => (s.text || '').trim()).filter(Boolean).join('\n\n');

        response = await api.post('/lyrics/voice-sync-resegment', {
          audioUrl: originalAudioUrl,
          lyricsText,
          // mantém compat: se backend quiser usar as estrofes atuais
      ...
      if (response.data?.segments?.length > 0) {
        // Se veio resegmentação, substitui os blocos/estrofes do editor
        const serverStanzas = Array.isArray(response.data?.stanzas)
          ? response.data.stanzas
          : null;

        const baseStyle = stanzas?.[0] || {};

        const nextStanzas = (serverStanzas || stanzas).map((s, idx) => ({
          id: idx,
          text: String(s?.text || ''),

          // tempos serão aplicados pelos segmentos
          startTime: '00:00',
          endTime: '00:00',

          // mantém defaults/estilo
          fontSize: baseStyle.fontSize ?? 32,
          fontFamily: baseStyle.fontFamily ?? 'Montserrat',
          color: baseStyle.color ?? '#FFFFFF',
          outlineColor: baseStyle.outlineColor ?? '#000000',
          bold: baseStyle.bold ?? false,
          italic: baseStyle.italic ?? false,
          underline: baseStyle.underline ?? false,
          transition: baseStyle.transition ?? 'fade',
          transitionDuration: baseStyle.transitionDuration ?? 1,
          alignment: baseStyle.alignment ?? 'center',
          leadIn: baseStyle.leadIn ?? 0.5,
          lines: []
        }));
        ...
        setSyncStatus({
          type: 'success',
          message: serverStanzas
            ? `✅ Sincronização concluída! (${nextStanzas.length} estrofes re-separadas pela IA)`
