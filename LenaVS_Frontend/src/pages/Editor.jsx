    ...
    try {
      ...
      if (response.data?.segments?.length > 0) {
        ...
        const nextStanzas = stanzas.map((s, idx) => ({
          ...
          leadIn: s.leadIn ?? baseStyle.leadIn ?? 0.5,
          lines: []
        }));
        ...
        const maxAttempts = Number(report?.maxAttempts || 5);

        let message = '✅ Sincronização concluída preservando as estrofes atuais.';
        if (retriedCount > 0) {
          message += ` ${retriedCount} estrofe(s) exigiram nova tentativa automática.`;
        }
        if (fallbackCount > 0) {
          message += ` ${fallbackCount} estrofe(s) não encontradas com confiança após até ${maxAttempts} tentativas e receberam interpolação para ajuste manual.`;
        }
