...
const Editor = () => {
  ...
  const handleLyricsProcessed = useCallback((processedStanzas) => {
    ...
    setSyncStatus({
      type: 'info',
      message:
        `${normalized.length} blocos carregados. Agora você pode duplicar, reordenar, mover livremente no preview e ajustar o tamanho por lá também.`,
    });
  }, []);
  ...
  const handleStanzasChange = useCallback((nextStanzas) => {
    const normalized = normalizeStanzas(nextStanzas);
    setStanzas(normalized);
  ...
  const handleStanzaFontSizeChange = useCallback((stanzaId, nextFontSize) => {
    setStanzas((prev) =>
      prev.map((stanza) =>
        stanza.id === stanzaId
          ? {
  ...
  const handleStanzaScaleChange = useCallback((stanzaId, nextScale) => {
    setStanzas((prev) =>
      prev.map((stanza) =>
        stanza.id === stanzaId
          ? {
  ...
  const loadProjectIntoEditor = useCallback((project, successMessage) => {
    ...
    const normalizedStanzas = normalizeStanzas(config.stanzas || []);
      ...
      <div className="editor-main">
        ...
        <div className="preview-wrapper">
          <PreviewPanel
            ...
            onSelectStanza={(stanzaId) => handleSelectStanza(stanzaId, { source: 'preview', shouldSeek: false, notifyIfMissingStart: false })}
            onStanzaPositionChange={handleStanzaPositionChange}
            onStanzaFontSizeChange={handleStanzaFontSizeChange}
            onStanzaScaleChange={handleStanzaScaleChange}
          />
        </div>
