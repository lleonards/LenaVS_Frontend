...
import { getActiveStanzaAtTime, hasDefinedStartTime, normalizeStanzas } from '../utils/stanza';
...
const Editor = () => {
  ...
  const requestPreviewSeek = useCallback((stanzaId, options = {}) => {
    ...
    if (!hasDefinedStartTime(stanza, { allowFirstBlockAtZero: stanzaIndex === 0 })) {
      if (notifyIfMissingStart) {
        setSyncStatus({
          type: 'warning',
          message: 'O preview só volta automaticamente quando o bloco tiver um início válido.',
        });
      }
      return false;
    }
  ...
  const focusCurrentStanza = useCallback(() => {
    if (!stanzas.length) return;

    const activeStanzaId = getActiveStanzaAtTime(stanzas, currentTimeRef.current)?.id ?? null;
    const targetId = activeStanzaId || selectedStanzaId || stanzas[0]?.id || null;

    if (!targetId) return;

    handleSelectStanza(targetId, {
      source: 'shortcut',
      shouldSeek: false,
      notifyIfMissingStart: false,
    });
    focusSelectedStanzaBlock();
  }, [focusSelectedStanzaBlock, handleSelectStanza, selectedStanzaId, stanzas]);
  ...
  }, [focusCurrentStanza, focusSelectedStanzaBlock, handleTapSync, navigateBetweenStanzas, selectedStanzaId]);
