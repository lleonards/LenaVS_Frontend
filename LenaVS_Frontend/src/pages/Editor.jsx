...
const Editor = () => {
  ...
  // Chamado pelo LyricsEditorPanel quando o usuário clica em "Marcar"
  ...
  const handleAutoSync = async () => {
    ...
    try {
      // ── Tenta sincronização via Whisper no backend ─────────────────
      if (originalAudioUrl) {
        try {
          const hasTwoAudios = Boolean(mediaFiles?.musicaOriginal && mediaFiles?.musicaInstrumental);

          const response = hasTwoAudios
            ? await api.post('/lyrics/voice-sync-auto', {
                audioCandidates: [mediaFiles.musicaOriginal, mediaFiles.musicaInstrumental],
                stanzas: stanzas.map(s => ({ id: s.id, text: s.text }))
              })
            : await api.post('/lyrics/voice-sync', {
          ...
          if (response.data?.segments?.length > 0) {
            ...
            const updated = stanzas.map((s, idx) => {
              // ✅ CORRIGIDO: interpola corretamente em vez de usar último segmento
              const seg = segments[idx];

              if (!seg) {
                // Interpola entre o segmento anterior e próximo disponível
                const prevSeg = segments[idx - 1];
                const nextSeg = segments.find((_, i) => i > idx && segments[i]);
              ...
              return {
                ...
                leadIn: 0.5,
                // ✅ NOVO: guarda timestamps por linha para karaokê line-level
                lines: seg.lines || []
              };
            });
      ...
      // ── Fallback: distribuição proporcional por palavras ───────────
      ...
      // Conta palavras por linha em cada estrofe
      const allLines = [];
      ...
      const updated = stanzas.map((s, si) => {
