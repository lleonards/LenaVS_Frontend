...
import { formatTimecode, DEFAULT_TIMECODE } from '../utils/timecode';
import './Editor.css';
...
const Editor = () => {
  ...
  const secondsToTimecode = (sec) => formatTimecode(sec, { decimals: 2 });
  ...
  const applySegmentsToStanzas = useCallback((currentStanzas, segments) => {
    return currentStanzas.map((s, idx) => {
      ...
      return {
        ...s,
        startTime: secondsToTimecode(seg.start),
        endTime:   secondsToTimecode(seg.end),
        leadIn: seg.showOnlyDuringVocal ? 0 : 0.5,
        showOnlyDuringVocal: Boolean(seg.showOnlyDuringVocal),
        syncConfidence: typeof seg.confidence === 'number' ? seg.confidence : null,
        syncVerified: Boolean(seg.verified),
        lowConfidence: Boolean(seg.lowConfidence),
        vocalPresence: Boolean(seg.vocalPresence),
        lines: Array.isArray(seg.lines) ? seg.lines : []
      };
    });
  }, []);

  const buildAutoSyncMessage = useCallback((analysis, totalStanzas) => {
    if (!analysis) {
      return '✅ Sincronização concluída!';
    }

    const verified = Number(analysis.verifiedCount || 0);
    const lowConfidence = Number(analysis.lowConfidenceCount || 0);
    const repaired = Number(analysis.repairedByAnalysis || 0);
    const repeated = Number(analysis.repeatedAdjustments || 0);
    const speechRefinements = Number(analysis.speechRefinements || 0);
    const total = Number(totalStanzas || analysis.stanzaCount || 0);

    let message = `✅ Sincronização concluída! ${verified}/${total} estrofes verificadas no tempo do vocal.`;

    if (repeated > 0) {
      message += ` Anti-falha aplicou ${repeated} ajuste(s) em trechos repetidos.`;
    }

    if (repaired > 0) {
      message += ` Contra-análise corrigiu ${repaired} trecho(s).`;
    }

    if (speechRefinements > 0) {
      message += ` Detecção de silêncio refinou ${speechRefinements} trecho(s) sem vocal.`;
    }

    if (lowConfidence > 0) {
      message += ` ${lowConfidence} estrofe(s) ficaram com baixa confiança para revisão manual.`;
    }
  ...
  const applyProportionalFallback = useCallback((currentStanzas, duration) => {
    ...
    return currentStanzas.map((s, si) => {
      ...
      return {
        ...s,
        startTime: secondsToTimecode(stanzaLines[0].start),
        endTime:   secondsToTimecode(stanzaLines[stanzaLines.length - 1].end),
        leadIn: 0.5,
        lines: []
      };
    });
  ...
  const handleAutoSync = async () => {
    ...
    try {
      // ── Sincronização via backend (usa APENAS a música ORIGINAL) ──
      // O backend cria internamente uma guia vocal para detectar o tempo do canto,
      // mas respeita exatamente as estrofes já separadas no editor.
      let response;
      try {
      ...
      if (response.data?.segments?.length > 0) {
        const baseStyle = stanzas?.[0] || {};

        const nextStanzas = stanzas.map((s, idx) => ({
          id: s.id ?? idx,
          text: String(s?.text || ''),
          startTime: DEFAULT_TIMECODE,
          endTime: DEFAULT_TIMECODE,
          fontSize: s.fontSize ?? baseStyle.fontSize ?? 32,
          fontFamily: s.fontFamily ?? baseStyle.fontFamily ?? 'Montserrat',
          color: s.color ?? baseStyle.color ?? '#FFFFFF',
          outlineColor: s.outlineColor ?? baseStyle.outlineColor ?? '#000000',
          bold: s.bold ?? baseStyle.bold ?? false,
          italic: s.italic ?? baseStyle.italic ?? false,
          underline: s.underline ?? baseStyle.underline ?? false,
          transition: s.transition ?? baseStyle.transition ?? 'fade',
          transitionDuration: s.transitionDuration ?? baseStyle.transitionDuration ?? 1,
          alignment: s.alignment ?? baseStyle.alignment ?? 'center',
          leadIn: 0,
          lines: []
        }));

        const updated = applySegmentsToStanzas(nextStanzas, response.data.segments);
        setStanzas(updated);
        setShowLyricsEditor(true);

        setSyncStatus({
          type: 'success',
          message: buildAutoSyncMessage(response.data?.analysis, updated.length)
        });
        return;
      }
