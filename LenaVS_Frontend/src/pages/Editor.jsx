...
const Editor = () => {
  const { user, loading, credits, hasUnlimitedAccess } = useAuth();
  ...
  const showCreditsExhaustedBanner = !hasUnlimitedAccess && Number(credits) <= 0;
  ...
  return (
    <div className="editor-container">
      ...
      {showCreditsExhaustedBanner ? (
        <div className="editor-upgrade-banner">
          <div>
            <strong>Seus 3 créditos gratuitos acabaram.</strong>
            <p>Ative o plano Unlimited para continuar exportando, subir arquivos e usar a biblioteca sem bloqueios.</p>
          </div>
          <button type="button" className="editor-upgrade-banner__button" onClick={() => navigate('/upgrade')}>
            Quero Unlimited
          </button>
        </div>
      ) : null}
