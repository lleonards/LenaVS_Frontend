...
const Editor = () => {
  ...
  const applySegmentsToStanzas = useCallback((currentStanzas, segments) => {
    return currentStanzas.map((s, idx) => {
      ...
      return {
        ...
        leadIn: 0.5,
        // timestamps por linha para karaokê line-level
  ...
  const handleAutoSync = async () => {
    ...
    try {
      // ── Tenta sincronização via backend (Whisper / silence-detect / proporcional) ──
      const hasTwoAudios = Boolean(mediaFiles?.musicaOriginal && mediaFiles?.musicaInstrumental);

      let response;
      try {
        response = hasTwoAudios
      ...
      if (response.data?.segments?.length > 0) {
        ...
        const method = response.data.method || 'automático';
        setSyncStatus({
          type: 'success',
          message: `✅ Sincronização concluída! (${method})`
        });
        return;
      }
