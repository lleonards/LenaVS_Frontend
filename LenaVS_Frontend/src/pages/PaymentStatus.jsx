...
const PaymentStatus = () => {
  ...
  useEffect(() => {
    ...
    const shouldSyncAccess = provider === 'sellx' ? status !== 'failure' : status === 'success';
      ...
      try {
        ...
        for (let attempt = 0; attempt < 8; attempt += 1) {
          const response = await api.get('/payment/subscription', {
            params: provider === 'sellx' ? { sync_provider: 'sellx' } : {},
          });
  ...
  const providerLabel = provider === 'sellx' ? 'SellX' : 'Stripe';
  const providerIcon = provider === 'sellx' ? <QrCode size={16} /> : <CreditCard size={16} />;
