...
export const STANZA_TRANSITION_DURATION_MIN = 0.5;
...
const measureWrappedText = (stanza, fontSize, previewWidth = 1280) => {
  const usableWidth = Math.max(80, previewWidth - 48);
  const maxWidth = Math.min(usableWidth, Math.max(80, previewWidth * 0.82));
...
export const getMaxSafeFontSizeForStanza = (stanza, options = {}) => {
  ...
  const maxTextHeight = Math.max(56, Math.min(previewHeight * 0.5, previewHeight - 48));
  ...
  while (low <= high) {
    ...
    const estimatedHeight = (metrics.lineCount * middle * 1.45) + 28;
    const fitsWidth = metrics.maxLineWidth + 36 <= metrics.allowedWidth;
